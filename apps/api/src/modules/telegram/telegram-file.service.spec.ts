...
describe('TelegramFileService', () => {
  ...
  it('maps --local absolute file_path to nginx /file/bot URL on :8082', () => {
    const { service } = createService({
      TELEGRAM_BOT_TOKEN: token,
      TELEGRAM_API_BASE_URL: 'http://localhost:8081',
    });

    const { url } = service.buildFileDownloadUrl(
      `/var/lib/telegram-bot-api/${token}/videos/file_0`,
    );

    expect(url).toBe(`http://localhost:8082/file/bot${token}/videos/file_0`);
  });
